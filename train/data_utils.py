...
def prepare_dataset(num_samples: int = 60000) -> None:
	...
	train_images_int = tf.random.uniform((num_samples, 28, 28), 
	                                     minval=0, maxval=256, 
	                                     dtype=tf.int32, seed=42)
	train_images = tf.cast(train_images_int, tf.uint8)
	
	test_images_int = tf.random.uniform((num_samples // 6, 28, 28),
	                                   minval=0, maxval=256,
	                                   dtype=tf.int32, seed=43)
	test_images = tf.cast(test_images_int, tf.uint8)
	
	train_labels = tf.random.uniform((num_samples,), 
	                                minval=0, maxval=10,
	                                dtype=tf.int32, seed=44)
	test_labels = tf.random.uniform((num_samples // 6,),
	                               minval=0, maxval=10,
	                               dtype=tf.int32, seed=45)
	
	log(f"Dataset loaded: {num_samples:,} training images, {num_samples // 6:,} test images")
	...
	log(f"  Pixel value range: [{tf.reduce_min(train_images)}, {tf.reduce_max(train_images)}]")
	...
	log(f"  Normalized range: [{tf.reduce_min(train_images_norm):.6f}, {tf.reduce_max(train_images_norm):.6f}]")
	...
	log(f"  Class distribution: {dict(zip(unique_classes.numpy(), class_counts.numpy()))}")
	time.sleep(0.3)
	...
	log(f"  Train tensor shape: {train_images_split.shape}, memory: {train_memory_mb:.1f} MB")
	log(f"  Validation tensor shape: {val_images_split.shape}, memory: {val_memory_mb:.1f} MB")
...
def build_model_architecture() -> None:
	...
	sample_input = tf.random.uniform((batch_size, *input_shape), dtype=tf.float32, seed=42)
	...
	log(f"  Input statistics: mean={tf.reduce_mean(sample_input):.6f}, std={tf.math.reduce_std(sample_input):.6f}")
	time.sleep(0.2)
	
	# Layer 1: Conv2D
	log("Conv2D_1: 32 filters, kernel=(3,3), stride=1, padding='same'")
	log("  Weight shape: (3, 3, 1, 32) | Bias: (32,)", level="DEBUG")
	conv1_layer = layers.Conv2D(32, (3, 3), padding='same', activation='relu', name='conv2d_1')
	...
	log(f"  Output statistics: mean={tf.reduce_mean(conv1_output):.6f}, std={tf.math.reduce_std(conv1_output):.6f}")
	...
	conv2_layer = layers.Conv2D(32, (3, 3), padding='same', activation='relu', name='conv2d_2')
	...
	log(f"  Pooled output: mean={tf.reduce_mean(pool1_output):.6f}, max={tf.reduce_max(pool1_output):.6f}")
	time.sleep(0.12)
	...
	active_neurons = tf.reduce_sum(tf.cast(tf.not_equal(dropout1_output, 0.0), tf.float32))
	total_neurons = tf.size(dropout1_output, out_type=tf.float32)
	log(f"  Active neurons: {active_neurons:.0f}/{total_neurons:.0f} ({active_neurons/total_neurons:.2%})")
	time.sleep(0.12)
	
	# Layer 3: Conv2D
	log("Conv2D_3: 64 filters, kernel=(3,3), stride=1, padding='same'")
	log("  Weight shape: (3, 3, 32, 64) | Bias: (64,)", level="DEBUG")
	conv3_layer = layers.Conv2D(64, (3, 3), padding='same', activation='relu', name='conv2d_3')
	...
	conv4_layer = layers.Conv2D(64, (3, 3), padding='same', activation='relu', name='conv2d_4')
	...
	log(f"  Pooled output: mean={tf.reduce_mean(pool2_output):.6f}, max={tf.reduce_max(pool2_output):.6f}")
	time.sleep(0.12)
	...
	active_neurons = tf.reduce_sum(tf.cast(tf.not_equal(dropout2_output, 0.0), tf.float32))
	...
	log(f"  Pre-activation: mean={tf.reduce_mean(dense1_output):.6f}, std={tf.math.reduce_std(dense1_output):.6f}")
	...
	active_neurons = tf.reduce_sum(tf.cast(tf.not_equal(dropout3_output, 0.0), tf.float32))
	...
	log(f"  Logits: mean={tf.reduce_mean(logits):.6f}, std={tf.math.reduce_std(logits):.6f}, "
	...
	total_params = conv1_params + conv2_params + conv3_params + conv4_params + dense1_params + dense2_params
	log("Model summary:")
	log(f"  Total parameters: {total_params:,} (trainable: {total_params:,}, non-trainable: 0)")
	log(f"  Model size: ~{(total_params * 4) / (1024*1024):.1f} MB (float32 weights)")
	log(f"  Memory footprint per batch: ~{(batch_size * total_params * 4) / (1024*1024):.2f} MB")
	time.sleep(0.2)

...
export class HomePage implements OnInit, AfterViewInit {
  ...
  isModelAvailable(_modelType: ModelType): boolean {
    return true;
  }
  ...
  displayPreprocessedImage(imageData: ImageData): void {
    ...
    const tensor = tf.tidy(() => {
      ...
      return tf.browser.fromPixels(imageData, 1).resizeNearestNeighbor([28, 28]).expandDims(0).div(255.0);
    });
    ...
    // tfjs types + TS strictness can make these appear as possibly-undefined
    const height = preprocessedTensor.shape[0] ?? 28;
    const width = preprocessedTensor.shape[1] ?? 28;
